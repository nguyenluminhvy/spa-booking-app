...
export type DashboardAdvancedFilterValue = {
  type: string | undefined;
  range: DateRange | undefined;
};

interface AdvancedFilterModalProps {
  value: DashboardAdvancedFilterValue;
  onChange: (data: DashboardAdvancedFilterValue) => void;
}
...
export function DashboardAdvancedFilterModal({ onChange, value }: AdvancedFilterModalProps) {
  const [visible, setVisible] = useState(false);
  const [filters, setFilters] = useState<DashboardAdvancedFilterValue>({
    type: 'all',
    range: {
      start: moment().startOf('month').toDate(),
      end: moment().endOf('month').toDate(),
  ...
  const hasFilter = !!filters.type || isSwitchOn;
          ...
          >
            ...
            >
              {STATE_BUTTONS.map((button, index) => {
                const isActive = button.type === filters.type;
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setFilters(prev => ({ ...prev, type: isActive ? undefined : button.type }));
                    }}
                  >
